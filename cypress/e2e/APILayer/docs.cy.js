...
describe('Test Cases for Docs/ Documentation', () => {
    before(() => {
    ...
    it('2. Test to click on "Docs" link and check if it is redirecting', () => {
        cy.scrollTo('bottom');
        cy.get('footer') // Get the footer
            .find('.nav-item') // Narrow down to the specific column
            .contains('a', 'Docs') // Find the link with the text 'Docs'
            .should('be.visible') // Ensure it is visible
            .click(); // Click it
        cy.contains('How can we help?', { timeout: 7000 }).should('be.visible');
    });

    it('3. Test to Click on "Account" Card', () => {
        cy.get('.row').find('a[href="/docs/listing/account"]').first().click();
    });
