...
describe('Image Optimizer API Documentation page', () => {
    ...
    it('14. Verify the "GET /url" endpoint content and click "Subscribe for Free"', () => {
        ...
        cy.get('#endpointCollapse1 .card-body p')
            .contains('Optimizes and reduces file size of the given URL.')
            .should('be.visible');
        ...
        // Check if the 2nd 'code-box' exists
    ...
    it('15. Verify the "POST /upload" endpoint content and click "Subscribe for Free"', () => {

        // Verify the POST /upload heading is present and visible
        cy.get('#endpointHeading2 button')
            .find('i.fas.fa-angle-right')
            .should('be.visible');

        // Verify Parameters section header
        cy.get('#endpointCollapse2 .card-body h6')
            .contains('Parameters')
            .should('be.visible');

        // Verify 'body' parameter section
        cy.get('p.mb-2')
            .first() // Get the first matching element
            .should('be.visible')
            .within(() => {
        ...
        // Verify and click the 'Subscribe for Free' button
        cy.get('#subscribeButton')
            .should('be.visible')
            .and('contain.text', 'Subscribe for Free')
            .click();

        // Verify that clicking the button activates the pricing tab
        cy.get('a#pricing-tab')
            .should('have.class', 'active')
            .and('be.visible');

        // Verify Documentation link and click it
        cy.get('.d-md-flex > .d-none').contains('Documentation').click();
    });
});
