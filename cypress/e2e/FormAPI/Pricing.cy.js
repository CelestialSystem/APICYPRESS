...
describe('Form API Pricing page', () => {
    ...
    it('5. Test to check "MOST POPULAR" label on Starter Plan', () => {
    ...
    it('7. Test to check "Pro Plan" price is $39.99', () => {
        cy.componentVisiblityCheck('.card > .card-header > .mb-3 > .plan-price', '$19.99');
        cy.componentVisiblityCheck('.media-body');
    });

    it('8. Test to check "Subscribe" button click and "Pro Plan" features (50,000 requests, standard support)', () => {
        cy.wait(500);
    ...
    it('10. Test to check features of "Custom plan" is present', () => {
        cy.get('.media-body').contains(' Any requests volume you need ').should('be.visible');
    });
 
    it('11. Test to check "Custom Plan" features', () => {
        cy.componentVisiblityCheck('.card > .card-header > .h3', 'Custom Plan');
